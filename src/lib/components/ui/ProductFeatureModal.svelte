<script lang="ts">
  ...
  let save = async () => {
    ...
    if (res.status === 200) {
      ...
      $productFeatures = [...$productFeatures.map((obj) => obj.id == patchedProductFeature.id ? patchedProductFeature : obj)];
  ...
  $: title = productFeature ? productFeature.implementation_status == 'NOT_AVAILABLE' ? 'N/A' : productFeature.id : 'N/A';
</script>
